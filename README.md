...
- `/v1/images/generations` 图片生成，模型列表包含 `gpt-image-2.5`，支持 SSE 流式返回，支持 URL 或 `b64_json`。
...
在 Railway 为服务添加 Volume，并将挂载路径设为 `/data`。镜像会在 `/data` 运行，账号 token、账号元数据和管理台 API 密钥都会写入该 Volume；重新部署后仍会保留。
